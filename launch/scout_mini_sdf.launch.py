...
def generate_launch_description():
	...
	gazebo_models_path = 'description/scout_mini/meshes'
	package_name = 'tangent_bug_pmr'
	robot_name_in_model = 'scout_mini'
	rviz_config_file_path = 'config/config.rviz'
	model_file_path = 'description/scout_mini/model.sdf'
	...
	default_urdf_model_path = os.path.join(pkg_share, model_file_path)
	...
	start_robot_state_publisher_cmd = Node(
		...
		parameters=[{'robot_description': launch_ros.descriptions.ParameterValue( launch.substitutions.Command([
			'xacro ',os.path.join(pkg_share,model_file_path)]), value_type=str)  }]
	)
	...
	spawn_entity_cmd = Node(
		...
		arguments=['-entity', robot_name_in_model, 
								'-file', os.path.join(pkg_share,model_file_path),
										'-x', spawn_x_val,
										'-y', spawn_y_val,
										'-z', spawn_z_val,
										'-Y', spawn_yaw_val],
	...
	# 	name='ekf_node',
	# 	output='screen',
	# 	parameters=[os.path.join(FindPackageShare(package='scout_gazebo_sim').find('scout_gazebo_sim'), 'config', 'ekf.yaml'), {'use_sim_time': LaunchConfiguration('use_sim_time')}]
	# )
	...
	# ld.add_action(start_joint_state_publisher_gui) # WIP: use ros2 run joint_state_publisher_gui joint_state_publisher_gui
